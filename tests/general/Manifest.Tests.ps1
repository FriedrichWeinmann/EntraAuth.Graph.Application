Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\EntraAuth.Graph.Application").Path
	$manifest = ((Get-Content "$moduleRoot\EntraAuth.Graph.Application.psd1") -join "`n") | Invoke-Expression
